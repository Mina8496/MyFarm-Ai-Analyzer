...
import 'package:myfarm/common/helpers/typing_text_helper.dart';
import 'package:myfarm/features/plant_analysis/Presentation/widgets/custom_appbar.dart';
...
class _TFLiteClassifierPageState extends State<TFLiteClassifierPage> {
  ...
  Future<void> _pickImageAndClassify() async {
    if (!_classifier.isModelLoaded) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Model is still loading, please wait...'.tr)),
      );
      return;
    }
      ...
      if (result.confidence < 75.0) {
        setState(() {
          _result = ClassificationResult(
            ...
            confidence: 0.0,
            topResults: {"Healthy": 0.0, "Gall Midge": 0.0, "Sooty Mould": 0.0},
          );
          _typingHelper.textNotifier.value =
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF388E3C),
      appBar: CustomAppBar(title: 'Plant Analysis'.tr),
      body: Container(
              ...
              children: [
                Align(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: _classifier.isModelLoaded
  ...
  Widget getHeader(String label) {
    ...
    if (normalized == 'gall midge') {
      header =
          'Gall midge causes blister-like swellings on mango leaves, leading to tissue damage';
    } else if (normalized == 'healthy') {
      header =
          'A healthy mango leaf is vibrant green, smooth, and free from spots or deformities.';
    } else if (normalized == 'sooty mould') {
      header =
          'Sooty mould appears as a black, powdery coating on mango leaves, blocking sunlight and affecting photosynthesis.';
    } else {
      header = '';
    }
